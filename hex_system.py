...
class LatticeSystem:
    ...
    def __init__(self, dim, A, AA, I, J, II, JJ, III, JJJ):
        ...
        self.P, self.box = hex_lattice.fill_box(dim)
        self.dim = dim
    ...
    def show(self):
        ...
        self.create_mdist()
        hex_inout.show_system(self.P, self.II, self.JJ, self.D, self.ll,
                          box=self.box)

    def show2(self):
        """Plot system configuration."""
        self.create_mdist()
        hex_inout.show_system2(self.P, self.I, self.J, self.II, self.JJ, self.III,
    ...
    def save(self, name):
        ...
        self.create_mdist()
        hex_inout.save_system(name, self.P, self.II, self.JJ, self.D, self.ll,
                          box=self.box)

    def save2(self, name, maxv):
        """Save system configuration."""
        self.create_mdist()
        hex_inout.save_system2(name, self.P, self.I, self.J, self.II, self.JJ,
...
def stretch_sys(sys, alpha, ax=1):
    ...
    sys_s = LatticeSystem(sys.dim, sys.A, sys.AA, sys.I, sys.J, sys.II, sys.JJ,
                          sys.III, sys.JJJ)
    sys_s.P, sys_s.box = hex_lattice.stretch_it(sys_s.P, sys_s.box, alpha,
                                                axis=ax)
    return sys_s
